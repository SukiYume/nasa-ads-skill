...
安装器在 Windows 创建 `%LOCALAPPDATA%\nasa-ads\bin\adslib.cmd` 并补充用户 PATH；macOS/Linux/WSL 创建 `~/.local/bin/adslib`，按需为当前 bash、zsh 或 sh 配置追加带标记的 PATH 段。已有设置会保留，同名第三方命令会保留并提示冲突。自主管理 PATH 时使用 `install --bin-dir <目录> --no-path`，再把该目录加入自己的 PATH。
